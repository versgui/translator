...
class DeeplTranslatorTranslator extends HttpTranslator implements TranslatorService
{
    ...
    public function __construct($key, HttpClient $httpClient = null, RequestFactory $requestFactory = null)
    {
        ...
        if (empty($key)) {
            throw new \InvalidArgumentException('Deepl "key" can not be empty');
        }
    ...
    public function translate($string, $from, $to)
    {
        ...
        foreach ($data['data']['translations'] as $translaton) {
            return $this->format($string, $translaton['text']);
        }
    }
    ...
    {
        return sprintf(
            'https://api.deepl.com/v2/translate?auth_key=%s&source_lang=%s&target_lang=%s&text=%s',
            $key,
            $from,
            $to,
            urlencode($string)
